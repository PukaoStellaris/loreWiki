  ...
  characters: [
    {
      ...
      image: 'https://cdn.discordapp.com/attachments/1264685436878717219/1451469218099757181/icon2_whitebg.png?ex=6946498c&is=6944f80c&hm=17345ed5b6dde5a085986d5410bd150a4c6a5603b0c902f8efb9d8d19616fe34&',
      infobox: {
...
Her ultimate technique, "Shattering Heaven and Earth," allows her to fracture the very air itself, creating devastating shockwaves that bypass conventional defenses.`,
    ...
    },
    {
      ...
      category: 'Main Characters',
      image: 'https://cdn.discordapp.com/attachments/1264685436878717219/1451469217512689664/commis183.png?ex=6946498c&is=6944f80c&hm=7dd5e10e5f511f24cde2a0013aeb2999cae5cf44546981e4758bf72bd3ffbc8f&',
      infobox: {
        'Full Name': 'Sentinel',
        'Title': 'Archmage',
  ...
  <nav className="flex items-center space-x-2 text-sm mb-6 flex-wrap">
    {items.map((item, index) => (
      <React.Fragment key={index}>
        {index > 0 && <ChevronRight className="w-4 h-4 text-amber-600/50" />}
        <button
          onClick={() => item.path && onNavigate(item.path)}
          className={index === items.length - 1 ? 'text-amber-300 font-medium' : 'text-amber-500/70 hover:text-amber-300 transition-colors'}
        >
          {item.label}
        </button>
      </React.Fragment>
...
const Infobox = ({ title, image, data }) => (
  <div className="relative">
    <div className="absolute -top-1 -left-1 w-6 h-6 border-t-2 border-l-2 border-amber-500/60 rounded-tl-lg" />
    <div className="absolute -top-1 -right-1 w-6 h-6 border-t-2 border-r-2 border-amber-500/60 rounded-tr-lg" />
    <div className="absolute -bottom-1 -left-1 w-6 h-6 border-b-2 border-l-2 border-amber-500/60 rounded-bl-lg" />
    <div className="absolute -bottom-1 -right-1 w-6 h-6 border-b-2 border-r-2 border-amber-500/60 rounded-br-lg" />
    
    <div className="bg-gradient-to-br from-stone-900/95 via-stone-800/95 to-stone-900/95 border border-amber-700/40 rounded-xl overflow-hidden">
      <div className="bg-gradient-to-r from-amber-900/60 via-amber-800/40 to-amber-900/60 px-4 py-3 border-b border-amber-700/40">
        <h3 className="font-bold text-amber-200 text-center text-lg">{title}</h3>
      </div>
      <div className="p-4">
        <div className="w-full aspect-square bg-gradient-to-br from-stone-800 to-stone-900 rounded-lg flex items-center justify-center text-6xl mb-4 border border-amber-700/30">
          <img src={image} className="rounded-lg" />
        </div>
        <table className="w-full">
          <tbody>
            {Object.entries(data).map(([key, value]) => (
              <tr key={key} className="border-b border-amber-700/20 last:border-0">
                <th className="text-left py-2 pr-3 text-amber-400/80 text-sm font-medium align-top w-2/5">{key}</th>
...
const LoreCard = ({ item, onClick }) => (
  <button onClick={onClick} className="group w-full text-left relative overflow-hidden">
    <div className="absolute inset-0 bg-gradient-to-r from-amber-500/0 via-amber-500/10 to-amber-500/0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-xl" />
    <div className="relative bg-gradient-to-br from-stone-800/90 via-stone-800/80 to-stone-900/90 rounded-xl overflow-hidden border border-amber-700/30 group-hover:border-amber-500/50 transition-all duration-500 shadow-lg">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-20 h-0.5 bg-gradient-to-r from-transparent via-amber-500/50 to-transparent" />
      <div className="flex items-start p-5 gap-4">
        <div className="w-16 h-16 bg-gradient-to-br from-amber-900/50 to-stone-900 rounded-lg flex items-center justify-center text-3xl flex-shrink-0 group-hover:scale-110 transition-transform duration-500 border border-amber-700/30">
          <img src={item.image} className="rounded-lg" />
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="font-bold text-amber-100 text-lg group-hover:text-amber-300 transition-colors truncate">{item.name}</h3>
          {item.title && <p className="text-amber-500/80 text-sm mt-0.5 italic">{item.title}</p>}
          <div className="flex flex-wrap gap-2 mt-2">
            {item.type && <span className="px-3 py-1 bg-amber-900/40 text-amber-400 text-xs rounded-full border border-amber-700/30">{item.type}</span>}
            ...
            {item.category && <span className="px-3 py-1 bg-amber-900/40 text-amber-400 text-xs rounded-full border border-amber-700/30">{item.category}</span>}
          </div>
        </div>
        <ChevronRight className="w-5 h-5 text-amber-600/50 group-hover:text-amber-400 group-hover:translate-x-1 transition-all" />
      </div>
    </div>
  </button>
);
...
const SearchBar = ({ value, onChange, onMenuClick }) => (
  <div className="flex items-center gap-3 mb-6 sticky top-0 z-30 bg-gradient-to-b from-stone-900 via-stone-900/95 to-transparent pt-4 pb-8 -mt-4 -mx-4 px-4 md:-mx-8 md:px-8">
    <button onClick={onMenuClick} className="lg:hidden p-3 bg-stone-800 rounded-xl border border-amber-700/30 hover:border-amber-600/50 transition-all">
    ...
    <div className="relative flex-1">
      ...
      <input
        ...
        className="w-full pl-12 pr-12 py-3 bg-stone-800/80 border border-amber-700/30 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/50 transition-all placeholder-stone-500 text-stone-200"
      />
      <Sparkles className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-amber-600/30" />
    </div>
  </div>
);

const StatCard = ({ label, value, icon: Icon }) => (
  <div className="bg-gradient-to-br from-stone-800/90 to-stone-900/90 rounded-xl p-5 text-center border border-amber-700/20 hover:border-amber-600/40 transition-colors">
    <Icon className="w-8 h-8 mx-auto mb-2 text-amber-400/70" />
...
const CategoryCard = ({ title, subtitle, description, icon: Icon, onClick }) => (
  <button onClick={onClick} className="group relative text-left overflow-hidden rounded-2xl w-full">
    <div className="relative bg-gradient-to-br from-stone-800/90 via-stone-800/80 to-stone-900/90 p-6 border border-amber-700/30 group-hover:border-amber-500/50 transition-all duration-500 rounded-2xl">
      ...
      <div className="relative z-10 flex items-start gap-4">
        <div className="w-14 h-14 bg-gradient-to-br from-amber-700/40 to-amber-900/40 rounded-xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-500 border border-amber-600/30">
          <Icon className="w-7 h-7 text-amber-400" />
        </div>
        <div className="flex-1">
          <p className="text-amber-500/60 text-xs font-medium tracking-widest uppercase">{subtitle}</p>
          <h3 className="text-xl font-bold text-amber-100 group-hover:text-amber-200 transition-colors">{title}</h3>
          <p className="text-stone-400 mt-1 text-sm italic">{description}</p>
        </div>
        <ChevronRight className="w-6 h-6 text-amber-600/30 group-hover:text-amber-400 group-hover:translate-x-1 transition-all" />
      </div>
    </div>
  </button>
);
...
const Sidebar = ({ activeCategory, onSelectCategory, isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-72 bg-gradient-to-b from-stone-900 via-stone-900 to-stone-950 transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} flex flex-col shadow-2xl border-r border-amber-800/20`}>
        <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-amber-600/50 via-amber-400/50 to-amber-600/50" />
        <div className="p-6 border-b border-amber-800/30">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-gradient-to-br from-amber-700/50 to-amber-900/50 rounded-xl flex items-center justify-center shadow-lg border border-amber-600/30">
                <Scroll className="w-6 h-6 text-amber-400" />
              </div>
              <div>
                <h1 className="text-xl font-bold text-amber-200 tracking-wide">Lore Wiki</h1>
                <p className="text-amber-600/60 text-xs tracking-widest uppercase">Imperial Archives</p>
              </div>
            </div>
            <button onClick={onClose} className="lg:hidden p-2 text-amber-600 hover:text-amber-400 transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all relative ${isActive ? 'bg-gradient-to-r from-amber-800/40 to-amber-900/20 text-amber-200 border border-amber-600/30' : 'text-stone-400 hover:bg-stone-800/50 hover:text-amber-300 border border-transparent'}`}
              >
                ...
                {isActive && <Sparkles className="ml-auto w-4 h-4 text-amber-500/50" />}
              </button>
            );
          })}
        </nav>
        <div className="p-4 border-t border-amber-800/30">
          <div className="bg-stone-800/50 rounded-xl p-4 border border-amber-700/20">
            <div className="flex items-center gap-2 mb-2">
              <Eye className="w-4 h-4 text-amber-500/60" />
              <span className="text-amber-400/80 text-xs font-medium">Arcane Note</span>
            </div>
            <p className="text-stone-400 text-xs">Message <code className="text-amber-400/80 bg-stone-900 px-1 rounded">pukao</code> for questions.</p>
          </div>
        </div>
      </aside>
    </>
  ...
  return (
    <div className="space-y-8">
      <div className="relative overflow-hidden rounded-3xl">
        <div className="absolute inset-0 bg-gradient-to-br from-amber-900/30 via-stone-900 to-violet-900/30" />
        ...
        <div className="relative p-8 md:p-12 border border-amber-700/30">
          <div className="flex items-center gap-3 mb-4">
            <Moon className="w-5 h-5 text-violet-400" />
            <div className="w-16 h-px bg-gradient-to-r from-violet-400/50 to-transparent" />
            <span className="text-amber-400/80 text-sm font-medium tracking-widest uppercase">Imperial Archives</span>
            <div className="w-16 h-px bg-gradient-to-l from-amber-400/50 to-transparent" />
            <Sun className="w-5 h-5 text-amber-400" />
          </div>
          <h1 className="text-4xl md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-100 to-amber-200 mb-4">The Grand Chronicle</h1>
      ...
      <div>
        <div className="flex items-center gap-4 mb-6">
          ...
          <h2 className="text-2xl font-bold text-amber-200">Explore the Archives</h2>
      ...
      <div className="relative overflow-hidden rounded-2xl">
        <div className="absolute inset-0 bg-gradient-to-r from-violet-900/20 via-stone-900 to-amber-900/20" />
        <div className="relative p-6 border border-amber-700/30">
          <div className="flex items-center gap-2 mb-3">
            <Star className="w-5 h-5 text-amber-400" />
            <h3 className="text-lg font-bold text-amber-300">Featured: Grand Marshal Astral Anemos</h3>
          </div>
  ...
  return (
    <div>
      <div className="relative mb-8">
        <div className="absolute inset-0 bg-gradient-to-r from-amber-500/5 via-amber-500/10 to-amber-500/5 rounded-2xl blur-xl" />
        <div className="relative bg-gradient-to-br from-stone-800/80 to-stone-900/80 rounded-2xl p-6 border border-amber-700/30">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-gradient-to-br from-amber-700/50 to-amber-900/50 rounded-xl flex items-center justify-center shadow-lg border border-amber-600/30">
              <Icon className="w-7 h-7 text-amber-400" />
            </div>
            <div>
              <p className="text-amber-500/60 text-sm font-medium tracking-widest uppercase">{config.subtitle}</p>
              <h1 className="text-3xl font-bold text-amber-100">{config.title}</h1>
      ...
      {filteredData.length === 0 ? (
        <div className="text-center py-16 bg-stone-800/50 rounded-2xl border border-amber-700/20">
          <Search className="w-12 h-12 text-amber-600/30 mx-auto mb-4" />
          <p className="text-stone-400 text-lg">No ancient records found for "{searchQuery}"</p>
        </div>
      ) : groupedData ? (
        Object.entries(groupedData).map(([groupName, items]) => (
          <div key={groupName} className="mb-8">
            <h2 className="text-xl font-bold text-amber-300 mb-4 flex items-center gap-3">
              {groupName === 'Main Characters' && <Star className="w-5 h-5 text-amber-400" />}
...
const ArticleView = ({ item, category, onBack }) => {
  ...
  return (
    <div>
      <button onClick={onBack} className="flex items-center gap-2 text-amber-500 hover:text-amber-300 mb-4 transition-colors group">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <article className="flex-1 min-w-0">
          <div className="relative">
            <div className="absolute -top-2 -left-2 w-8 h-8 border-t-2 border-l-2 border-amber-500/40 rounded-tl-xl" />
            <div className="absolute -top-2 -right-2 w-8 h-8 border-t-2 border-r-2 border-amber-500/40 rounded-tr-xl" />
            <div className="absolute -bottom-2 -left-2 w-8 h-8 border-b-2 border-l-2 border-amber-500/40 rounded-bl-xl" />
            <div className="absolute -bottom-2 -right-2 w-8 h-8 border-b-2 border-r-2 border-amber-500/40 rounded-br-xl" />
            
            <div className="bg-gradient-to-br from-stone-800/95 via-stone-800/90 to-stone-900/95 rounded-2xl shadow-2xl border border-amber-700/30 overflow-hidden">
              <div className="relative bg-gradient-to-r from-amber-900/50 via-amber-800/30 to-amber-900/50 px-8 py-6 border-b border-amber-700/30">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 bg-gradient-to-br from-amber-800/60 to-stone-900 rounded-xl flex items-center justify-center text-5xl border border-amber-600/40 shadow-lg">
                  <img src={item.image} className="rounded-lg" />
                  </div>
                  <div>
                    <h1 className="text-3xl font-bold text-amber-100 tracking-wide">{item.name}</h1>
                    {item.title && <p className="text-amber-400/80 text-lg mt-1 italic">{item.title}</p>}
                  </div>
                </div>
              </div>
              ...
              <div className="p-8">
                ...
                <div className="mt-6 space-y-4">
                  {item.description.split('\n\n').map((p, i) => (
                    <p key={i} className={`text-stone-300 leading-relaxed ${i === 0 ? 'first-letter:text-2xl first-letter:text-amber-400 first-letter:mr-1' : ''}`}>{p}</p>
                  ))}
                </div>
                ...
                {item.abilities && (
                  <div className="mt-8 pt-6 border-t border-amber-700/30">
                    <h2 className="text-xl font-bold text-amber-200 mb-4 flex items-center gap-2">
                      <Sparkles className="w-5 h-5 text-amber-500" /> Abilities & Techniques
                    </h2>
                    <ul className="space-y-3">
                      {item.abilities.map((ability, i) => (
                        <li key={i} className="flex items-start gap-3 text-stone-300 bg-stone-900/50 rounded-lg p-3 border border-amber-700/20">
                  ...
                  <div className="mt-8 pt-6 border-t border-amber-700/30">
                    ...
                    <div className="grid gap-3">
                      {item.relationships.map((rel, i) => (
                        <div key={i} className="flex items-center gap-3 bg-stone-900/80 rounded-lg px-4 py-3 border border-amber-700/20">
                          <span className="font-semibold text-amber-300">{rel.name}</span>
